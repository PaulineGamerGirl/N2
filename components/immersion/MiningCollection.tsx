...
const MiningCollection: React.FC = () => {
  ...
  const groupedData = useMemo(() => {
    ...
    filtered.forEach(card => {
      ...
      if (groupBy === 'ANIME') {
        ...
        // Map generic titles to "Other Immersion"
    ...
    // Sort: If date mode, sort groups by actual timestamp descending. 
    // If anime mode, sort alphabetically, but keep "Other Immersion" at bottom.
    return Object.entries(groups).sort(([keyA, cardsA], [keyB, cardsB]) => {
      if (groupBy === 'DATE') {
        return cardsB[0].timestamp - cardsA[0].timestamp;
      }
  ...
  const selectAllVisible = () => {
    const allVisibleIds = groupedData.flatMap(([_, cards]) => cards.map(c => c.id));
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      await exportToAnki(cardsToExport);
      ...
      <div className="p-6 border-b border-rose-100 flex flex-col md:flex-row items-center justify-between bg-white sticky top-0 z-20 shadow-sm gap-6">
        <div className="flex items-center gap-6 flex-1 w-full">
          ...
          <div className="flex bg-rose-50 p-1 rounded-2xl border border-rose-100 shrink-0">
             <button 
              onClick={() => setGroupBy('ANIME')}
              className={`flex items-center gap-2 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${groupBy === 'ANIME' ? 'bg-white text-rose-500 shadow-sm' : 'text-rose-300 hover:text-rose-400'}`}
             >
               <Film className="w-3 h-3" /> By Anime
             </button>
             <button 
              onClick={() => setGroupBy('DATE')}
              className={`flex items-center gap-2 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${groupBy === 'DATE' ? 'bg-white text-rose-500 shadow-sm' : 'text-rose-300 hover:text-rose-400'}`}
             >
               <History className="w-3 h-3" /> By Day
             </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
          <div className="flex items-center gap-2">
            {totalSelected > 0 && (
              <button 
                ...
                className="text-[9px] font-black uppercase tracking-widest text-rose-300 hover:text-rose-500 transition-colors px-2"
              >
                Clear Selection
              </button>
            )}
            <div className="text-[10px] font-black uppercase tracking-widest text-coquette-subtext bg-rose-50 px-3 py-1.5 rounded-full border border-rose-100">
              {totalSelected > 0 ? `${totalSelected} Selected` : `${minedCards.length} Cards Total`}
            </div>
          </div>
          ...
          <button 
            ...
            className={`
              flex items-center gap-2 px-6 py-2.5 rounded-2xl font-black text-[10px] uppercase tracking-[0.15em] transition-all shadow-md active:scale-95
                ...
                <div key={title} className="space-y-4">
                  ...
                  <div className="flex items-center justify-between px-2">
                    <div className="flex items-center gap-4 flex-1">
                      <button 
                        onClick={() => toggleGroup(cardIds)}
                        className={`p-1.5 rounded-lg transition-all ${allInGroupSelected ? 'text-rose-500' : 'text-gray-300 hover:text-rose-300'}`}
                        title="Toggle Entire Section"
                      >
                        {allInGroupSelected ? <CheckSquare className="w-5 h-5" /> : (someInGroupSelected ? <Layers className="w-5 h-5 text-rose-400" /> : <Square className="w-5 h-5" />)}
                      </button>
                      <h2 className="text-lg font-bold font-coquette-header text-gray-700 uppercase tracking-[0.15em] italic flex items-center gap-3">
                        {groupBy === 'ANIME' ? <Film className="w-4 h-4 text-rose-300" /> : <Calendar className="w-4 h-4 text-rose-300" />}
                        {title}
                        <span className="text-[10px] lowercase font-sans opacity-40 not-italic tracking-normal ml-1">({cards.length})</span>
                      </h2>
                        ...
                        {cards.map((card) => {
                          ...
                          return (
                            <motion.tr 
                              ...
                              className={`cursor-pointer transition-colors group ${isCardSelected ? 'bg-rose-50/50' : 'hover:bg-rose-50/20'}`}
                            >
                              <td className="px-6 py-5">
                                 <div className={`transition-all ${isCardSelected ? 'text-rose-500' : 'text-gray-200 group-hover:text-rose-200'}`}>
                                    {isCardSelected ? <CheckCircle2 className="w-5 h-5" /> : <Square className="w-5 h-5" />}
                                 </div>
                              </td>
                              <td className="px-6 py-5">
                                <span className={`text-lg font-bold font-coquette-body transition-colors ${isCardSelected ? 'text-rose-700' : 'text-gray-800'}`}>{card.front}</span>
                              </td>
                              <td className="px-6 py-5">
                                <span className="text-sm text-gray-500 font-coquette-body italic">{card.translation}</span>
                              </td>
                              <td className="px-6 py-5 hidden lg:table-cell">
                                <p className="text-xs text-gray-400 line-clamp-1 max-w-sm leading-relaxed">{card.back}</p>
                              </td>
                              <td className="px-6 py-5 text-center">
                                {card.image && (
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); setSelectedImage(card.image); }}
                                    className="p-2.5 rounded-xl bg-white border border-gray-100 text-gray-400 hover:text-rose-500 hover:border-rose-200 transition-all shadow-sm"
                                    title="View Scene"
                                  >
                              ...
                              <td className="px-6 py-5 text-center">
                                {card.audio && (
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); playAudio(card.audio); }}
                                    className="p-2.5 rounded-xl bg-white border border-gray-100 text-gray-400 hover:text-emerald-500 hover:border-emerald-200 transition-all shadow-sm"
                                    title="Play Snippet"
                                  >
        ...
        {selectedImage && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 backdrop-blur-md bg-black/80">
            <motion.div 
              ...
              className="relative max-w-5xl w-full aspect-video rounded-[32px] overflow-hidden shadow-2xl border-4 border-white/10"
            >
              <img src={selectedImage} alt="Mined Evidence" className="w-full h-full object-cover" />
              <button 
                onClick={() => setSelectedImage(null)}
                className="absolute top-6 right-6 p-3 rounded-full bg-black/40 text-white hover:bg-black/60 transition-colors backdrop-blur-md"
              >
                <X className="w-6 h-6" />
              </button>
            </motion.div>
