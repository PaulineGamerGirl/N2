...
/**
 * Senior Data Engineer: Anki Sync Engine (Schema V4)
 * Targets Note Type with exactly: 
 * Word; Word Meaning; Reading; Sentence Field; Image Field; Audio Field; Tags
 */
export const exportToAnki = async (
  cards: MinedCard[], 
  readings: Record<string, string> = {}, 
  deckName: string = "NihongoNexus_Export"
) => {
  ...
  csvRows.push("#columns:Word;Word Meaning;Reading;Sentence Field;Image Field;Audio Field;Tags");

  console.log(`[AnkiExport] Deploying Schema V4 with Readings for ${cards.length} nodes...`);
  ...
  for (const card of cards) {
    ...
    try {
      ...
      const row = [
        ...
        (readings[card.id] || '').replace(/;/g, ','), // The New Reading Field
  ...
  try {
    ...
    link.href = url;
    link.download = `${deckName}_${new Date().toISOString().split('T')[0]}.apkg.zip`;
